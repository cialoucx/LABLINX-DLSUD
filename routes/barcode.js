...
const { INCIDENT_DEADLINE_MS } = require("../config/constants");
const {
  logAdminAction,
  checkStockAndNotify,
...
router.post("/api/return-by-barcode", isAdmin, async (req, res) => {
  try {
    ...
    if (condition === "Damaged" || condition === "Lost") {
      ...
      const deadlineAt = new Date(Date.now() + INCIDENT_DEADLINE_MS);
      ...
      if (responsibleUser.email) {
        const emailSubject = "Incident Report Required - Action Required";
        const emailBody = `
          ...
          <p><strong>IMPORTANT:</strong> You must submit an incident report within <strong>48 hours</strong> of this notification. Failure to do so will result in suspension from borrowing equipment.</p>
