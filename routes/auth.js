...
const recordFailedLogin = (key) => {
  ...
  if (current.count >= maxAttempts) {
    current.lockedUntil = new Date(Date.now() + lockoutMinutes * 60 * 1e3);
  }
...
router.post("/login", async (req, res) => {
  ...
  if (remainingLockMs > 0) {
    const remainingMinutes = Math.ceil(remainingLockMs / (60 * 1e3));
    return res
      .status(429)
      .send(
  ...
  req.logout(function (err) {
    ...
    req.session.destroy((err) => {
      ...
      if (isStudentOrFaculty && tenantID) {
        ...
        const msLogoutUrlWithLogin = `https://login.microsoftonline.com/${tenantID}/oauth2/v2.0/logout?post_logout_redirect_uri=${encodeURIComponent(loginRedirectUri)}`;
...
router.get(
  ...
  (req, res, next) => {
    if (!passport._strategies || !passport._strategies.microsoft) {
      console.warn(
        "[AUTH WARN] Microsoft SSO strategy is not registered. Redirecting to login with error.",
      );
