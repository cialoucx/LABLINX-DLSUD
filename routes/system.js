...
const getSettingsForAdmin = async (adminUsername) => {
  ...
  const values = await Promise.all(
    keys.map((key) => getSettingValue(key, { adminUsername: adminUsername })),
  );
  const settingsObj = {};
...
router.put("/api/system-settings/:key", isAdmin, async (req, res) => {
  try {
    ...
    const allowedSettingRules = {
      ...
      reservation_daily_rate: { min: 0, max: 1e6 },
    ...
    const { min: min, max: max } = allowedSettingRules[key];
    ...
    broadcastRefresh();
    res.json({ message: "Setting updated.", setting: setting });
  } catch (error) {
...
router.get("/api/borrowing-info", isAuthenticated, async (req, res) => {
  try {
    ...
    res.json({
      ...
      replacementReturnDays: replacementReturnDays,
