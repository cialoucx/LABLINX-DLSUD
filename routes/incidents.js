  ...
  async (req, res) => {
    try {
      ...
      const reports = await StudentIncidentReport.find({ studentId })
        .populate({ path: "incidentId", options: { strictPopulate: false } })
        .sort({ deadlineAt: 1 })
        .lean();
      res.json(reports || []);
    } catch (error) {
    ...
    try {
      ...
      if (existingReport) {
        ...
        const newReport = new StudentIncidentReport({
          ...
          deadlineAt:
            incident.deadlineAt || new Date(Date.now() + INCIDENT_DEADLINE_MS),
          status: "Submitted",
        });
  ...
  async (req, res) => {
    try {
      ...
      await logAdminAction(
        ...
        `Rejected incident report ${report._id} by Admin 2. Reason: ${
          rejectionNote || "None specified"
        }`,
      );
      broadcastRefresh();
      res.json({
