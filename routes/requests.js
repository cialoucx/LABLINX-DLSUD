  ...
  try {
    ...
    if (startDate && dueDate) {
      const daysDiff = Math.ceil(
        (new Date(dueDate) - new Date(startDate)) / MS_PER_DAY,
      );
...
router.put("/api/edit-request/:id", isAdmin, async (req, res) => {
  try {
    ...
    request.set({ ...otherUpdates, quantity: newQuantity || oldQuantity });
    await request.save();
...
router.put("/api/update-request/:id", isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (wasPending && isApproved) {
      ...
      if (!updatedItem) {
        return res.status(409).json({
          message: "Cannot approve request. Failed to update item stock.",
        });
      }
