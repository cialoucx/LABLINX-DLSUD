...
const {
  findModelAndItemForAdmin: findModelAndItemForAdmin,
...
const {
  Inventory: Inventory,
  ScienceInventory: ScienceInventory,
  SportsInventory: SportsInventory,
  FurnitureInventory: FurnitureInventory,
  ComputerInventory: ComputerInventory,
  FoodLabInventory: FoodLabInventory,
  RoboticsInventory: RoboticsInventory,
  MusicInventory: MusicInventory,
  allInventoryModels: allInventoryModels,
    ...
    try {
      ...
      if (duplicateInBatch.length > 0) {
        ...
        return res.status(409).json({
          message: `Duplicate Item IDs found in the request: ${uniqueDuplicates.join(", ")}. Each Item ID must be unique (case-insensitive).`,
        });
      }
      ...
      if (existingItems.length === 0) {
        ...
        const regexMatches = await Model.find({ $or: existingItemsQueries });
      ...
      if (
        e.code === 11e3 ||
        (e.writeErrors && e.writeErrors.some((err) => err.code === 11e3))
      ) {
        const duplicateKey =
          e.keyValue?.itemId ||
...
router.put("/api/inventory/restore/:itemId", isAdmin, async (req, res) => {
  try {
    const { item: item } = await findModelAndItemForAdmin(
      req.params.itemId,
...
router.delete("/api/inventory/permanent/:itemId", isAdmin, async (req, res) => {
  try {
    const { item: item, Model: Model } = await findModelAndItemForAdmin(
      req.params.itemId,
      req.session.user.username,
    );
    if (!item) {
