document.addEventListener("DOMContentLoaded", () => {
  ...
  const scheduleRealtimeRefresh = (reason = "event") => {
    ...
    realtimeRefreshDebounceTimer = setTimeout(() => {
      refreshRealtimeData({ reason: reason });
    }, 250);
  };
  const connectWebSocket = () => {
    ...
    socket.onclose = () => {
      ...
      wsReconnectTimer = setTimeout(connectWebSocket, 3e3);
    };
  };
    ...
    grid.innerHTML = days
      .map((day) => {
        ...
        const classes = ["due-day-card", count > 0 ? "has-due" : ""]
          .filter(Boolean)
          .join(" ");
        return `\n                <div class="${classes}">\n                  <div class="due-day-label">${day.toLocaleDateString("en-US", { weekday: "short" })}</div>\n                  <div class="due-day-date">${day.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</div>\n                  <div class="due-day-count">${count > 0 ? `${count} due` : "No due"}</div>\n                </div>\n              `;
      })
      .join("");
  };
  const renderUpcomingDeadlines = () => {
    ...
    deadlinesList.innerHTML = approvedRequests
      .map((req) => {
        ...
        dueDate.setHours(0, 0, 0, 0);
        const daysDiff = Math.floor((dueDate - now) / (1e3 * 60 * 60 * 24));
  ...
  const populateFilters = () => {
    ...
    categoryButtonsContainer.innerHTML = categories
      .map(
        (cat) =>
          `<button class="btn btn-category ${cat === "All" ? "active" : ""}" data-category="${cat}">${cat}</button>`,
      )
      .join("");
    const locations = [
      "All",
  ...
  const renderEquipmentTable = () => {
    ...
    groupedMap.forEach((group, key) => {
      ...
      renderEquipmentSummaryRow({
        ...
        aggregatedAvailable: aggregatedAvailable,
  ...
  const renderRequestsTable = () => {
    ...
    filtered
      ...
      .forEach((req) => {
        ...
        row.innerHTML = `\n                        <td><strong>${req.itemId}</strong></td><td>${req.itemName}</td><td>${startDate}</td>\n                        <td>${dueDate}</td><td><span class="status-badge ${statusClass}">${displayStatus}</span></td>\n                        <td>\n                            ${req.status === "Pending" ? `<button class="btn btn-cancel" onclick="cancelRequest('${req._id}')">Cancel</button>` : "—"}\n                        </td>\n                    `;
      });
  };
  ...
  const fetchMyNotifications = async () => {
    try {
      ...
      if (
        ...
        const notificationList = document.getElementById("notification-list");
        if (notificationList) {
          notificationList.innerHTML = `<p style="color: var(--text-subtle);">Error: ${error.message || "Failed to load notifications."}</p>`;
        }
      }
    }
  };
  const renderNotifications = () => {
    ...
    sortedNotifications.forEach((n) => {
      ...
      item.innerHTML = `\n                            <div class="notification-title">${sanitizeHtml(n.title || "No Title")}</div>\n                            <div class="notification-message">${sanitizeHtml(n.message || "No message")}</div>\n                            <div class="notification-time">${n.createdAt ? new Date(n.createdAt).toLocaleString() : "Unknown date"}</div>\n                            <div class="notification-actions">\n                                <button class="btn-delete-notification" onclick="deleteNotification('${n._id}')">Delete</button>\n                            </div>\n                        `;
  ...
  const showToast = (message, type = "info") => {
    ...
    const duration = type === "error" || type === "warning" ? 5e3 : 3e3;
    setTimeout(() => {
  ...
  const fetchPendingReports = async () => {
    try {
      ...
      const pendingList = document.getElementById("pending-reports-list");
      if (pendingList) {
        pendingList.innerHTML = `<p style="color: var(--text-subtle);">Error: ${error.message || "Failed to load pending reports."}</p>`;
      }
      showToast(
